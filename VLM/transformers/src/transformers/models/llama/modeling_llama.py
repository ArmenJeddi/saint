...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        cos, sin = self.rotary_emb(value_states, seq_len=5000)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.saint_image_token_length = config.saint_image_token_length
        self.saint_start = config.saint_start
        self.saint_end = config.saint_end
        self.saint_threshold = config.saint_threshold
    
    def set_saint(self):
        self.use_saint = self.config.use_saint
        self.saint_sys_length = self.config.saint_sys_length
        self.saint_image_token_length = self.config.saint_image_token_length
    ...
    def forward(
        ...
        USE_SAINT= self.use_saint
        SYS_LENGTH= self.saint_sys_length
        IMAGE_TOKEN_LENGTH = self.saint_image_token_length
        SAINT_START = self.saint_start
        SAINT_THRESH = self.saint_threshold
        SAINT_END = self.saint_end
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if USE_SAINT:
                ...
                if hidden_states.shape[1] > 1: ## prefilling         
                    if SAINT_START <= idx <= SAINT_END:
                        ...
                        tail_indices = torch.arange(SYS_LENGTH + IMAGE_TOKEN_LENGTH, seq_length, device=device).unsqueeze(0).expand(batch_size, -1)
                        
                        keep_indexs = torch.cat([system_indices, kept_vision_indices, tail_indices], dim=1)
                        keep_indexs, _ = torch.sort(keep_indexs, dim=1)
                        
                        seq_length = keep_indexs.shape[1]
                        batch_idx = torch.arange(batch_size, device=device).unsqueeze(1).expand(batch_size, seq_length)
                        hidden_states = hidden_states[batch_idx, keep_indexs, :]

                        position_ids = position_ids[batch_idx, keep_indexs]
